...
import jp.kentan.studentportalplus.databinding.ItemSmallLectureCancellationBinding
...
class LectureCancellationAdapter(
    ...
    override fun getItemViewType(position: Int) = when (currentList[position]) {
        ...
        is Item -> R.layout.item_small_lecture_cancellation
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        ...
        return when (viewType) {
            ...
            R.layout.item_small_lecture_cancellation -> ViewHolder.LectureCancellationViewHolder(
                ItemSmallLectureCancellationBinding.inflate(inflater, parent, false)
            )
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        when (holder) {
            is ViewHolder.HeaderViewHolder -> holder.binding.executeAfter {
                ...
                textView.apply {
                    ...
                    if (header.hasMoreItem) {
                        val suffix =
                            context.getString(R.string.suffix_more_item, header.moreItemCount)
                        append(suffix)
                    }
                }
            }
            is ViewHolder.LectureCancellationViewHolder -> holder.binding.executeAfter {
                ...
                data = item.lectureCancel
                layout.setOnClickListener { onItemClick(item.lectureCancel.id) }
            ...
            is ViewHolder.ShowAllViewHolder -> holder.binding.executeAfter {
                textView.apply {
                    setText(R.string.action_show_all_lecture_cancellation)
    ...
    fun submitList(list: List<LectureCancellation>) {
        ...
        // item
        if (list.isNotEmpty()) {
            newList.addAll(list.take(MAX_ITEM_COUNT).mapIndexed { index, lectureCancel ->
                Item(
                    lectureCancel = lectureCancel,
    ...
    private data class Item(
        val lectureCancel: LectureCancellation,
    ...
    private class Callback(
        ...
        override fun areItemsTheSame(oldItemPosition: Int, newItemPosition: Int): Boolean {
            ...
            return when {
                ...
                oldItem is Item && newItem is Item -> oldItem.lectureCancel.id == newItem.lectureCancel.id
    ...
    sealed class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        class LectureCancellationViewHolder(
            val binding: ItemSmallLectureCancellationBinding
        ) : ViewHolder(binding.root)
